...
if __name__ == "__main__":
    ...
    logger.info("Running config file %s...", "run_only_hp_control")

    nameDeck = "run_only_hp_control"
    pathBase = os.getcwd()

    runTool = runTrnsys.RunParallelTrnsys(pathBase, nameDeck)

    runTool.readConfig(pathBase, "run_only_hp_control.config")
    runTool.getConfig()
    runTool.runConfig()
    runTool.runParallel()

    logger.info("...DONE (%s).", "run_only_hp_control")